...
get_mass_matrix <- function(stanfit, nChains) {
  adapt_info <- get_adaptation_info(stanfit)
  string <- "# Step size = 0.00654987\n# Diagonal elements of inverse mass matrix:\n#"
  start <- nchar(string) + 1
  
  mass_matrix <- list()
  for (i in 1:nChains) {
    end <- nchar(adapt_info[i]) - nchar("\n# ")
    mass_matrix_chain <- substring(adapt_info[i], start, end)
    mass_matrix[[i]] <- 
      as.numeric(strsplit(mass_matrix_chain, split =", ")[[1]])
  }
  mass_matrix
}
